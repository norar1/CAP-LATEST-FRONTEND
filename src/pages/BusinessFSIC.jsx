...
function FSIC({ onUpdateStats }) {
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    const confirmMessage = `Are you sure you want to ${newStatus === 'approved' ? 'approve' : newStatus === 'rejected' ? 'reject' : 'set to pending'} the permit for ${business?.business_name}?${newStatus !== 'pending' ? '\n\nAn email notification will be sent to the user.' : ''}`;
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/businessfsic/data/UpdateStatus/${id}`, {
        method: 'PUT',
        headers: {
      ...
      <div className="bg-white p-4 md:p-6 rounded-lg shadow-lg w-full max-w-6xl mx-4 max-h-screen overflow-y-auto">
        <div className="flex justify-between items-center mb-4 sticky top-0 bg-white z-10 pb-2 border-b">
          <h3 className="text-lg md:text-xl font-bold text-blue-700">
            {editingId ? 'Update Business Permit' : 'Create Business Permit'}
          </h3>
          <button 
            type="button"
        ...
        <form onSubmit={handleSubmit} className="mt-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-gray-700 mb-1 text-sm">BIN #</label>
              <input
                type="text"
                name="bir_tin"
    ...
    <>
      ...
      <div className="bg-white p-4 md:p-6 rounded-lg shadow">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 space-y-4 md:space-y-0">
          <h3 className="text-lg md:text-xl font-bold text-blue-700">Business Permit</h3>
          ...
          <table className="min-w-full divide-y divide-gray-300 table-fixed border-collapse">
            <thead className="bg-gray-100">
              <tr className="border-b border-gray-300">
                ...
                <th className="px-2 md:px-3 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider w-24 border-r border-gray-200">BIN #</th>
